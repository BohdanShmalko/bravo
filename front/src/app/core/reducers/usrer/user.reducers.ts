...
import {UserActions, userActionsType} from "@core/reducers/usrer/user.actions";
...
export const userReducer: ActionReducer<UserState, UserActions> =
  (state: UserState = initialState, action: UserActions): UserState => {
    switch (action.type) {
      case userActionsType.changeCurrentPage: return { ...state, currentPage: action.payload }
      case userActionsType.changeMenuOpen: return { ...state, isOpenMenu: !state.isOpenMenu }
      default:
        return state;
    }
  }
